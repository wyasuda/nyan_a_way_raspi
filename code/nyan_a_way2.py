...
classifier2 = caffe.Classifier('/home/pi/deploy_v1.1.prototxt',
    '/home/pi/SqueezeNet_iter_4500.caffemodel',
    image_dims=[227,227], mean=np.load('/home/pi/mean.npy'),
    raw_scale=255.0,
    channel_swap=[2,1,0])
...
for f in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    if too_big_flg == False:
        ...
        if out_label == 'cat'  and 0.99 < prob:
            cat_cnt1 = cat_cnt1 + 1
            print'cat_cnt1 = '+str(cat_cnt1)

            t_width = (max_x-min_x)*1.1
            t_height = (max_y-min_y)*1.1
            min_x = int(round(max(0, tx-t_width/2)))
            max_x = int(round(min(width, tx+t_width/2)))
            min_y = int(round(max(0, ty-t_height/2)))
            max_y = int(round(min(width, ty+t_height/2)))
            dst = frame[min_y:max_y, min_x:max_x]
            cv_img = dst.copy()
            cv_img = cv2.cvtColor(cv_img, cv2.COLOR_BGR2RGB)
            cv_float = cv_img.astype(np.float32)
            cv_float /= 255.0
            predictions2 = classifier2.predict([cv_float], False)
            prob2 = predictions2[0].max()
            out_label2 = labels[predictions2[0].argmax()]
            if out_label2 == 'cat'  and 0.99 < prob2:
                cat_cnt2 = cat_cnt2 + 1
                print'cat_cnt2 = '+str(cat_cnt2)

                cv2.putText(frame, "Probability:"+str(prob2), (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
                cv2.putText(frame, str(datetime.datetime.today()), (S_Width-240, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
                cv2.rectangle(frame, (min_x, min_y), (max_x, max_y), (0, 0, 255), 2)
                print '*** Target detected! ' + str(prob2) + " : " + out_label + " ***"

                tmpstr = cat_detected_time.strftime('value1=detect%Y%m%d_%H%M%S.%f.jpg')+',cat_cnt1:'+str(cat_cnt1)+',cat_cnt2:'+str(cat_cnt2)
